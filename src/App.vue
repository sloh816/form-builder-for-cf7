<template>
    ...
    <p class="fixed bottom-6 left-6 text-sm text-slate-600">Made with 💜 by <a href="#" class="text-indigo-700 underline">Shannon L.</a><br />GitHub | Report a problem</p>
</template>
...
function duplicateForm() {
    if (currentForm.value) {
        ...
        currentForm.value.fields.forEach((field) => {
            if (field.props.id) {
                field.props.id = field.props.id.split("-")[0] + "-" + crypto.randomUUID().substring(2, 8);
            }
        });
...
provide("duplicateField", (fieldId: string) => {
    ...
    if (currentForm.value) {
        const field: FormField | undefined = currentForm.value.fields.find((f) => f.props.id === fieldId);
        if (field) {
            // Create a deep copy of the field
            const newField: FormField = JSON.parse(JSON.stringify(field));
            newField.props.id = field.props.id.split("-")[0] + "-" + crypto.randomUUID().substring(2, 8); // Generate a new ID for the duplicated field
...
provide("addStyle", (style: Style) => {
    if (currentForm.value) {
        ...
        const existingStyleIndex = currentForm.value.styles.findIndex((s) => s.label === style.label);
