<template>
    <div class="form-info bg-slate-200 min-w-[300px] p-4 flex flex-col gap-6">
        <div class="flex flex-col">
            ...
            <input id="form-name" type="text" placeholder="Untitled form..." class="bg-white p-2 rounded text-sm border border-slate-300 mx-2 block" :value="props.currentFormId" @input="handleInput(($event.target && ($event.target as HTMLInputElement).value) || '')" />
        </div>
