        ...
        <template #preview>
            <label>
                ...
                <span class="wpcf7-form-control-wrap" data-name="textarea-210">
                    <textarea cols="40" :maxlength="props.maxlength ?? undefined" :rows="props.rows" class="wpcf7-form-control wpcf7-textarea pointer-events-none" aria-invalid="false" name="textarea-210" :placeholder="props.placeholder" disabled></textarea>
                </span>
            </label>
        </template>
        <template #options>
            <FieldOptionsFormWrapper :id="props.id" fieldType="Text area">
                ...
                <NumberInput :id="`id-${props.id}`" label="Rows" :value="props.rows || null" :inputFunction="(value: any) => updateProps('rows', value)" description="Height of the text area in lines" />

                <NumberInput :id="`id-${props.id}`" label="Max Length" :value="props.maxlength ?? null" :inputFunction="(value: any) => updateProps('maxlength', value)" description="Maximum number of characters allowed" />
