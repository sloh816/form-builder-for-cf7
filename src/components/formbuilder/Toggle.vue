    ...
    <FormPreviewField>
        <template #preview>
            ...
            <div class="indiga-toggle-switch" :data-field-id="props.id">
                <p class="indiga-label" :style="computedStyles">{{ props.label }}<span v-if="props.required">*</span></p>
                ...
                <span class="wpcf7-form-control-wrap" :data-name="props.id">
                    ...
                    <span class="wpcf7-form-control wpcf7-radio">
                        ...
                        <span class="wpcf7-list-item last">
                            <label>
                                <input type="radio" name="radio-18" :value="props.offLabel" checked />
                ...
                <div class="indiga-toggle-switch-button-wrapper" :data-checked="props.default ? 'true' : 'false'" style="display: none">
                    <button class="indiga-toggle-switch-button" :aria-label="`Toggle ${props.label} `" :aria-description="`Currently toggled: ${props.default ? props.onLabel : props.offLabel}`" type="button">
                        <span class="indiga-toggle-switch-button-inner"></span>
                    </button>
                    <span class="indiga-toggle-button-label">{{ props.default ? props.onLabel : props.offLabel }}</span>
                </div>
            </div>
        </template>
...
onMounted(() => {
    const toggle = document.querySelector(`.indiga-toggle-switch[data-field-id="${props.id}"]`) as HTMLElement;
    if (toggle) {
    ...
    // Re-add the function when the form is updated
    const updateFormField = inject<Function>("updateFormField");
    if (updateFormField) {
        updateFormField(props.id, props);
    }
});

function addFunctionToToggle(toggle: HTMLElement) {
    const toggleButtonWrapper = toggle.querySelector(".indiga-toggle-switch-button-wrapper") as HTMLElement;
    ...
    toggleButton.addEventListener("click", () => {
        ...
        const toggleButtonLabel = toggle.querySelector(".indiga-toggle-button-label") as HTMLElement;
        if (!isChecked === true) {
            const firstRadio = radioButtons.querySelector('.first input[type="radio"]') as HTMLInputElement;
            ...
            const lastRadio = radioButtons.querySelector('.last input[type="radio"]') as HTMLInputElement;
