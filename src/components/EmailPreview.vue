<template>
    <div class="max-w-3xl mb-16">
        <label class="flex gap-4 items-center mb-4">
            ...
            <input class="border border-slate-300 p-2 rounded w-full" type="text" placeholder="Email address to send to..." :value="props.currentForm.email" @input="updateEmail(($event.target as HTMLInputElement)?.value ?? '')" />
        </label>
        <label class="flex gap-4 items-center mb-4">
            <span class="w-30"><strong>Your domain:</strong></span>
            <input class="border border-slate-300 p-2 rounded w-full" type="text" placeholder="Your website domain... eg. example.com" :value="props.currentForm.domain" @input="updateDomain(($event.target as HTMLInputElement)?.value ?? '')" />
        </label>
        <label class="flex gap-4 items-center mb-4">
            <span class="w-30"><strong>Subject:</strong></span>
            <input class="border border-slate-300 p-2 rounded w-full" type="text" :value="subject" @input="updateSubject(($event.target as HTMLInputElement)?.value ?? '')" />
        </label>

        <h1 class="text-3xl font-bold mt-8 mb-4">{{ subject }}</h1>
        <textarea class="w-full min-h-20 hover:outline outline-indigo-600 rounded-sm outline-offset-4 leading-relaxed" name="intro-text">
...
The response is below:
        ...
        <div v-for="(field, index) in emailPreviewArray" :key="index">
            <component v-if="field.component === 'Heading'" :is="'h' + field.props.headingLevel" class="text-2xl font-bold mb-4 mt-8">
                {{ field.props.heading }}
            </component>

            <component v-if="field.component === 'Table'" is="table" class="w-full mb-8 border border-slate-600">
                <tr>
                ...
                <tr v-for="formField in field.props.fields" :key="formField.props.name">
                    <td class="font-bold py-2 px-3 border-b border-slate-600 border-r border-slate-600">
...
onMounted(() => {
    if (props.currentForm) {
        ...
        props.currentForm.fields.forEach((field, index) => {
            ...
            if (field.component === "FormTitle" || field.component === "TextBlock" || field.component === "SubmitButton") {
                pushLastTable();
                return;
