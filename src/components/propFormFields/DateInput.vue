<template>
	<div class="flex flex-col gap-1 w-full">
		...
		<div class="flex items-center gap-2">
			<input
				class="border border-[#ccc] p-2 rounded-sm max-w-[150px]"
				type="date"
				:value="props.value"
				@input="(e) => inputFunction((e.target as HTMLInputElement).value)"
				:id="props.id"
			/>
			<button class="text-sm text-slate-500 cursor-pointer" @click="clearDateInput">
				Clear
			</button>
		</div>
	</div>
